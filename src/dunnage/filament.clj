...
(def retry-sentinal (Object.))
(defn catch-wrapper [f ^IFn get-index ^"[Ljava.lang.Object;" ->error ^"[I" ->error-out]
  (fn [^"[Ljava.lang.Object;" state]
    (loop []
      (let [val (try
                  (f state)
                  (catch Throwable e
                    (let [current-idx (.invoke get-index state)]
                      (aset state 0 (aget ->error current-idx))
                      (aset state 1 mode-error)
                      (aset state (aget ->error-out current-idx) e)
                      retry-sentinal)))]
        (if (identical? val retry-sentinal)
          (recur)
          val)))))
(defn make-catching-runner [{:keys [^"[Lclojure.lang.IFn;" fns ^IFn ->fiber ^IFn unwrap-state
                                    ^IFn get-mode ^IFn get-index ^"[Ljava.lang.Object;" ->error
                                    ^"[I" ->error-out]
                             :as   compiled}]
  (catch-wrapper
    (fn real-fn [state]
...
(defn invoker-node
  ([^IFn f success-out success-idx in1]
   (fn [^"[Ljava.lang.Object;" x]
     (let [v (.invoke f (aget x in1))]
       (aset x 0 success-idx)
       (aset x success-out v))
     x))
  ([^IFn f success-out success-idx in1 in2]
   (fn [^"[Ljava.lang.Object;" x]
     (let [v (.invoke f (aget x in1) (aget x in2))]
       (aset x 0 success-idx)
       (aset x success-out v))
     x))
  ([^IFn f success-out success-idx in1 in2 in3]
   (fn [^"[Ljava.lang.Object;" x]
     (let [v (.invoke f (aget x in1) (aget x in2) (aget x in3))]
       (aset x 0 success-idx)
       (aset x success-out v))
     x))
  ([^IFn f success-out success-idx in1 in2 in3 in4]
   (fn [^"[Ljava.lang.Object;" x]
     (let [v (.invoke f (aget x in1) (aget x in2) (aget x in3) (aget x in4))]
       (aset x 0 success-idx)
       (aset x success-out v))
     x))
  ([^IFn f success-out success-idx in1 in2 in3 in4 in5]
   (fn [^"[Ljava.lang.Object;" x]
     (let [v (.invoke f (aget x in1) (aget x in2) (aget x in3) (aget x in4) (aget x in5))]
       (aset x 0 success-idx)
       (aset x success-out v))
